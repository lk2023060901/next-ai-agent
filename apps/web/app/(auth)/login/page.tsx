...
function OAuthIcon({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <button
      type="button"
      aria-label={label}
      className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] transition-colors hover:bg-[var(--surface)] hover:border-[var(--border-hover)]"
    >
      {children}
    </button>
  )
    ...
    <div className="flex flex-col items-center gap-8">
      ...
      >
        ...
        <div className="mb-6 flex rounded-xl bg-[var(--surface-2)] p-1">
          {([['email', '邮箱登录'], ['phone', '手机登录']] as const).map(([val, label]) => (
            <button
              key={val}
              type="button"
              onClick={() => setTab(val)}
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={cn(
                  ...
                  'border-[var(--border)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent)]/20',
                )}
              />
            </div>
          ) : (
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="flex justify-center gap-3">
            <OAuthIcon label="GitHub 登录"><GitHubIcon /></OAuthIcon>
            <OAuthIcon label="Google 登录"><GoogleIcon /></OAuthIcon>
            <OAuthIcon label="Apple 登录"><AppleIcon /></OAuthIcon>
            <OAuthIcon label="微信登录"><WeChatIcon /></OAuthIcon>
          </div>
        </div>
      </div>
